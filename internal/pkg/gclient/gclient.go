...
func (gc *GrafanaClient) generateTempDashboard(dashboardFilePath, folderUid string) (string, error) {
	...
	dashboard["title"] = fmt.Sprintf("%s (Gflow %s)", dashboardTitle, newUid)
	dashboard["version"] = 0
	dashboard["id"] = nil
	dashboard["description"] = fmt.Sprintf("Generated by gflow. Watcher for %s", dashboardTitle)
	...
	if folderUid != "" {
		...
		requestBody["message"] = fmt.Sprintf("Gflow preview dashboard for %s", dashboardTitle)
	}
